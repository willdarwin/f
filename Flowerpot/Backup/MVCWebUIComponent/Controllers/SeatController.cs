...
namespace MVCWebUIComponent.Controllers
{
    public class SeatController : Controller
    {
        ...
        public JsonResult SeatData()
        {
            ...
            Idea idea = ideaService.GetIdeaByName("SeatBooking");
            if (idea != null)
            {
                IdeaDetail ideaDetail = ideaService.GetIdeaData(idea.IdeaId, "", "", 0, 10000, null);
                foreach (Row row in ideaDetail.Rows)
                {
        ...
        public JsonResult NewBooking(string newBooking)
        {
            ...
            JArray array = JArray.Parse(newBooking);
            ...
            IIdeaService ideaService = PolicyInjection.Wrap<IIdeaService>(Container.Resolve<IdeaService>());
            IRowService rowService = PolicyInjection.Wrap<IRowService>(Container.Resolve<IRowService>());
            Idea idea = ideaService.GetIdeaByName("SeatBooking");
            PropertyInfo[] pis = typeof(Booking).GetProperties();
            ...
            string result = rowService.AddRow(row).ToString();
