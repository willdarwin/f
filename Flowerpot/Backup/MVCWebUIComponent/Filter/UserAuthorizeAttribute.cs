...
namespace MVCWebUIComponent.Filter
{
    public class UserAuthorizeAttribute : AuthorizeAttribute
    {
        ...
        {
            UserModel userModel = filterContext.HttpContext.Session["CurrentUser"] as UserModel;
            User user = null;
            if (userModel == null)
            {
            ...
            bool isAllowed = IsAllowed(user, controller, action);
            if (!isAllowed)
            {
        ...
        {
            bool result = false;
            Role role = UserService.GetRoleByUserId(user.UserId);
